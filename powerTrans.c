...
{
    int swg_prim;
float kva=7500,kvar,v_in_line=33,v_out_line=11000,k=0.39,v_out_phase,axial_length_prim,radial_length_prim,window_height,axial_height_prim,area_per_strip;
...
float flux_density=1.6,i_density=2.6,working_i_density,temp_rise=45,avg_temp_rise,tappings1=7.5,tappings2=0.41,conductor_area_prim,conductor_area_sec,i_in_phase,i_out_phase,v_per_turn;
float tapping_turn,plain_disc_no=56,tap_disc_no=12,sec_disc=60,turn_per_disc_prim,turn_per_disc_sec,turn_per_layer_prim,turn_per_layer_sec,strips_inparallel=6;
...
wire_length_prim=round(conductor_area_prim*.46);
wire_width_prim=conductor_area_prim*0.113285;
wire_length_tap=conductor_area_prim*0.3283*1.02;
wire_width_tap=conductor_area_prim*0.15656*1.02;
printf("wire dimention of primary=%0.1fx%0.1f mm\n",wire_length_tap,wire_width_tap);


conductor_area_prim_proposed=(wire_length_prim*wire_width_prim)-0.25;
printf("conductor_area_primary proposed=%0.2f mm2\n",conductor_area_prim_proposed);

working_i_density=i_in_phase/conductor_area_prim_proposed;
printf("Working current density=%0.3f A/mm2\n",working_i_density);
...
turn_per_disc_prim_tap=tapping_turn*2/tap_disc_no;
turn_per_disc_prim_plain=(prim_turn-tapping_turn*2)/plain_disc_no;
printf("Turn per primary disc plain=%0.0f\n",turn_per_disc_prim_plain);

turn_per_disc_prim=(turn_per_disc_prim_plain+turn_per_disc_prim_tap);
...
axial_length_prim=round((wire_length_prim+0.6)*plain_disc_no)+round((wire_length_tap+0.6)*tap_disc_no)+297;
printf("Axial length for primary coil=%0.2f mm\n",axial_length_prim);

radial_length_prim=round((wire_width_prim+0.6)*turn_per_disc_prim_plain);
printf("Radial length of primary coil=%0.2f mm\n",turn_per_disc_prim);

window_height=axial_length_prim+130+30;
printf("W/H=%0.2f mm\n",window_height);
...
strip_width=round(conductor_area_sec*0.108997);
printf("Strip length for secondary coil=%0.0f mm\n",strip_width);

strip_depth=conductor_area_sec*0.04657;
...
area_per_strip=((strip_depth*strip_width)-0.86)*2;
...
axial_height_sec=((strip_width+0.5)*disposition_width*(plain_disc_no+4))+230;//round((strip_width+0.6)*disposition_width*turn_per_layer_sec)+10;
printf("Axial height of secondary winding=%0.2f mm\n",axial_height_sec);

radial_height_sec=round((strip_depth+0.5)*disposition_depth*round(turn_per_disc_sec));
...
core_limb_center=out_dia_prim+23;
printf("Core limb center=%0.2f mm\n",core_limb_center);

width_step1=round(core_diameter-9);
...
printf("           mm      mm       mm                      kg\n\n");
for (no_of_step= 1; no_of_step< 14; no_of_step++) {
...
coil_weight_sec=3.15*mean_length_ofturn_sec*conductor_area_sec*spec_gravity_copper*0.000001*sec_turn;
...
tank_length=2*core_limb_center+out_dia_prim+2*65;
printf("Tank length=%0.2fmm\n",tank_length);

tank_breadth=out_dia_prim+65+115;
printf("Tank breadth=%0.2fmm\n",tank_breadth);

tank_width=55+window_height+2*width_step1+220+100+50;
...
rad_center_distance=tank_breadth*2;
